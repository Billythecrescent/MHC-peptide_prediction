...
# generating combined figures as per reviewer's request. these are the figures included in the manuscript
def overall_auc_binary_classification_and_response_based():
    """
    plots 2.1 + 3.1 --> 9
    """
    overall_auc_binary_classification_based_df_sorted = pd.read_csv('./results/2.overall_auc_binary_classification_based.tsv', sep='\t')
    overall_auc_response_based_df_sorted = pd.read_csv('./results/3.overall_auc_response_based.tsv', sep='\t')
    new_df = overall_auc_binary_classification_based_df_sorted.merge(overall_auc_response_based_df_sorted, left_on='method', right_on='method')
    new_df.rename(columns={'auc_x': 'Binary classification based', 'auc_y': 'Response based'}, inplace=True)
    methods = new_df['method'].tolist()

    ax = new_df.plot.bar(rot=0, color=['steelblue', 'indianred'], figsize=(10, 8))
    plt.ylabel('AUC', fontsize=12)
    plt.xlabel('Methods', fontsize=10)
    plt.yticks(np.arange(0.0, 1.1, step=0.1), fontsize=12)
    plt.xticks(np.arange(len(methods)), [method_names_proper[x] for x in new_df['method'].tolist()], rotation=-45, ha='left', rotation_mode="anchor", fontsize=12)
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, frameon=False, borderaxespad=0.)
    plt.tight_layout()
    plt.savefig('./results/9.overall_auc_binary_classification_and_response_based.png', bbox_inches="tight")

overall_auc_binary_classification_and_response_based()

def amount_of_peptides_to_capture_x_percent_epitopes_and_response():
    """
    plots 4.1_50 + 5.1_50 --> 3A
    plots 4.1_75 + 5.1_75 --> 3B
    plots 4.1_90 + 5.1_90 --> 3C
    """
    fig_num = 10
    for percent_value in x_percent_values_to_be_captured:
        percent_value = str(percent_value)
        amount_of_peptides_to_capture_x_percent_epitopes = pd.read_csv('./results/4.amount_of_peptides_to_capture_'+percent_value+'_percent_epitopes.tsv', sep='\t')
        amount_of_peptides_to_capture_x_percent_response = pd.read_csv('./results/5.amount_of_peptides_to_capture_'+percent_value+'_percent_response.tsv', sep='\t')

        new_df = amount_of_peptides_to_capture_x_percent_epitopes.merge(amount_of_peptides_to_capture_x_percent_response, left_on='method', right_on='method')
        new_df['%_peptides_x'] = new_df['%_peptides_x'].str.strip('%').astype(float)
        new_df['%_peptides_y'] = new_df['%_peptides_y'].str.strip('%').astype(float)
        new_df = new_df[['method','%_peptides_x','%_peptides_y']]
        new_df.rename(columns={'%_peptides_x': 'Epitopes', '%_peptides_y': 'Response'}, inplace=True)
        methods = new_df['method'].tolist()

        new_df.plot(kind='bar', x='method', y=['Epitopes', 'Response'], color=['steelblue', 'indianred'], figsize=(10, 8))
        plt.ylabel('% peptides (log scale)', fontsize=12)
        plt.xlabel('Methods', fontsize=10)
        plt.yticks(np.arange(0.0, 1.1, step=0.1), fontsize=12)
        plt.xticks(np.arange(len(methods)), [method_names_proper[x] for x in new_df['method'].tolist()], rotation=-45, ha='left', rotation_mode="anchor", fontsize=12)
        plt.legend(bbox_to_anchor=(1.05, 1), loc=2, frameon=False, borderaxespad=0.)
        plt.yscale('log')
        plt.ylim(0.01, 100.1)
        plt.gca().set_yticklabels(["{0:.1f}%".format(x * 1) for x in [0.001, 0.01, 0.1, 1.0, 10.0, 100.0]], fontsize=12)
        plt.title('Amount of peptides to capture '+percent_value+'% epitopes & response')
        plt.tight_layout()
        plt.savefig('./results/'+str(fig_num)+'.amount_of_peptides_to_capture_'+percent_value+'_percent_epitopes_and_response.png', bbox_inches="tight")
        fig_num += 1

amount_of_peptides_to_capture_x_percent_epitopes_and_response()

def amount_of_epitopes_and_response_captured_in_top_172_peptides():
    amount_of_epitopes_captured_in_top_172_peptides = pd.read_csv('./results/6.amount_of_epitopes_captured_in_top_172_peptides.tsv', sep='\t')
    amount_of_response_captured_in_top_172_peptides = pd.read_csv('./results/7.amount_of_response_captured_in_top_172_peptides.tsv', sep='\t')

    new_df = amount_of_epitopes_captured_in_top_172_peptides.merge(amount_of_response_captured_in_top_172_peptides,left_on='method', right_on='method')
    new_df['%_epitopes'] = new_df['%_epitopes'].str.strip('%').astype(float)
    new_df['%_response'] = new_df['%_response'].str.strip('%').astype(float)
    new_df = new_df[['method', '%_epitopes', '%_response']]
    new_df.rename(columns={'%_epitopes': 'Epitopes', '%_response': 'Response'}, inplace=True)
    methods = new_df['method'].tolist()

    new_df.plot(kind='bar', x='method', y=['Epitopes', 'Response'], color=['steelblue', 'indianred'], figsize=(10, 8))
    plt.ylabel('% epitopes/response', fontsize=12)
    plt.xlabel('Methods', fontsize=10)
    plt.yticks(np.arange(0.0, 100.1, step=10), fontsize=12)
    plt.xticks(np.arange(len(methods)), [method_names_proper[x] for x in new_df['method'].tolist()], rotation=-45, ha='left', rotation_mode="anchor", fontsize=12)
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, frameon=False, borderaxespad=0.)
    plt.ylim(0, 100.1)
    plt.gca().set_yticklabels(["{0:.1f}%".format(x * 1) for x in np.arange(0.0, 100.1, step=10)], fontsize=12)
    plt.title('Amount of epitopes & response captured by top 172 peptides')
    plt.tight_layout()
    plt.savefig('./results/13.amount_of_epitopes_and_response_captured_in_top_172_peptides.png', bbox_inches="tight")

amount_of_epitopes_and_response_captured_in_top_172_peptides()
